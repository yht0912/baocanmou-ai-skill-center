...
- 包参谋推荐分 v2：在公开安装量与社区热度之外，加入近期趋势、仓库维护、许可证清晰度、资料完整度和包参谋能力方向，同时保留独立的综合热度分。
- 探索页新增推荐等级、推荐分和中英文入选理由；自动生成中英文前 30 名观察摘要。
- GitHub Actions 每周一、周四自动复筛，先测试再验证 300 条目录结构，并限制单仓库最多入选 20 项。

### 调整

- 推荐榜仅作为发现目录，不会自动安装第三方 Skill；只有排名或元数据实质变化时才提交，避免时间戳造成无效更新。

### 修复

- 补齐 Rust 命令层对推荐分、等级、入选理由、复核标记和许可证字段的传递，并为旧版缓存目录提供兼容降级，避免进入热门技能库时出现整页空白。
- 正式发布构建会显式生成带签名的更新器产物，同时保持普通本地构建不依赖发布签名凭据。
- 启用包参谋自有 GitHub 更新端点与更新签名公钥；私钥和密码仅保存在 GitHub Secrets，不进入源码与知识库。
